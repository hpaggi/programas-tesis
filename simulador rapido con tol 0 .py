     ...
     #    print(" favs de OMEGA para decision",lista1[1:10])
          ...
          #       Resp.write(rr)
               ...
               #   print ("OMEGA consulta a "+str(lista[i-2])+" con calidad "+str(Qx1))
                         if m < MAXREGS+1:
                              niveles[1,lista2[i-2],m, decision]=nivel
                    #     T=t
                         if restantes[1] > 0  : # and MODO == "I" or MODO=="S":
                              ...
                              if i> 2 and m < MAXREGS + 1 :
                                   ...
                                   espera=int(random.random()*TIMEOUT)
                                   MENSAJES_ITER=MENSAJES_ITER+1
                                   tipo_mens[a+espera,1,lista2[i-2],m,decision]=1
                                   t_generac[1,lista2[i-2],m,decision,a+espera]=a
                                   t_entrada[1,lista2[i-2],m,decision, a]= a+espera
                                   print ("enero entrada de cons de OMEGA para ag", lista2[i-2], "mens", m)
                                   if (lista2[i-2], m,decision) in largo_cola:
                                        largo_cola[lista2[i-2], m, decision]= largo_cola[lista2[i-2], m, decision] +1
                                   else:
                                        largo_cola[1, m, decision]= 1
                                   niveles[1,lista2[i-2], m, decision]=nivel
                                   restantes[1]=restantes[1]-1  
                                   a=a+1                                   
                                   Qy=CONSULTAR(1, lista2[i-2], m, decision, niveles[1,lista2[i-2],m, decision], Qx1, Qy)  
          ...
          #    print (sorted(list(tipo_mens.keys())))
                    ...
                    if PRIMERO == 1:
                     ...
                     #    print(len(tipo_mens))
                         ...
                         t= a            
                    puntero=puntero+1   
                    print("LEI  xx", xx," a", ag, "tipo", tipo_mens[a,xx,ag,mm,campo], " para m", m, "puntero",puntero, "clave",cl1)
...
#  import pandas
 ...
 #    aq=2    
     ...
     #       TABLA_TO[iteracion,N_agentes,MAXREGS,MAXMENS]=cuenta_to
                    ...
                    s="RESULTADOS_"+str(N_agentes)+"_"+str(MAXMENS)+"_"+MODO+"_"+str(MAXIT)+"_"+str(MAXREGS)+"_"+str(N_favoritos)+"XXXXXX  00000"+".txt"
