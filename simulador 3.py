...
def CALCULO_Qy(X,Y,id_msg,id_campo,Qy):
  ...
  V=Max-Min
  q=1/(1+V)*Max
  R=T[X,Y,id_msg,id_campo]**2/S[X,Y,id_msg,id_campo]  # X responde a Y
  ...
  if Y == 1 :
    if T[Y,X,id_msg,id_campo]==0:  #X responde a Y o sea Y consulta a X
      ...
      if Qy > Qx[X,Y,id_msg,id_campo]:
        ...
        if Qy == Qx[X,Y,id_msg,id_campo]:
          EXITOS_A=EXITOS_A+1      
  print("Calculo Qy="+str(Qy)+ " id msg "+ "Qx "+str(Qx[X,Y,id_msg,id_campo])+" msg "+str(id_msg)+ " id campo " + str(id_campo))
  return
#************************************************************************************
...
def RESPUESTA_OMEGA(X,Y,id_msg,id_campo,Qx,Qy,N_agentes,tipo_mens, MODO, restantes):
 ...
 # CALCULO_Qy(X,Y,id_msg,id_campo,Qy)
  ...
  for i in range (1,N_agentes):
    ...
    for j in range(0,t):
      if  t_entrada [i,1,id_msg,id_campo,j] > maxt:
        print(t_entrada,"t es", str(t))
        maxt=t_entrada [i,1,id_msg,id_campo,j]  # que hora le pongo??????+++++++++ ojo, que considero tooodas las horas
  if TIMEOUT+maxt < t:
    TIMEOUTS=TIMEOUTS+1
  #else:
...
def RESPONDER(X,Y,id_msg,id_campo,Qx,Qy,N_agentes,tipo_mens,MODO, restantes,hora):
  ...
  if (X,Y,id_msg,id_campo,t) in tipo_mens:
      if tipo_mens[X,Y,id_msg,id_campo,hora]==2 and t_entrada [X,Y,id_msg,id_campo,hora] == t or tipo_mens[X,Y,id_msg,id_campo,hora]==1 and t_entrada [X,Y,id_msg,id_campo,hora] == t + TIMEOUT:
        if pendientes[X,id_msg,id_campo]== 0:
         ...
         # X manda resp a Y entonces para calcular calidad de Y deben haber respondido todos o ser time out de la consulta de Y a X
          if pendientes[Y,id_msg,id_campo]<1 or t_generac[X,Y,id_msg,id_campo]+TIMEOUT < t :     # t es el instante actual??? ++++++++++ 
            
            CALCULO_Qy_SIMPLE(X,Y,id_msg,id_campo,Qy)   # OJO CALCULAR QY solo cuando haya pasado el timeout de Y o cuando respondieron todos
      ...
      # si hay mensajes consulta a cada favorito
        if restantes[1] > 0:
         ...
         #     T=t
              ...
              CONSULTAR(1, lista[i-2], decision, id_msg, niveles[1,lista[i-2],id_msg, decision], Qy, Qx1, pendientes, N_campos, restantes)
           ...
           # for campo in range (1,N_campos+1):
            for campo in range (1,73): 
              if (xx, ag, m, campo,t+tt) in tipo_mens:
                ...
                if t_generac[xx, ag, m, campo,t+tt, tipo] + TIMEOUT > t+tt:  # o solo > t ??+++    tipo 1 o 2 +++++++++++++++++++++++++++++++++++ t_generac de tipo 1 es de cons sino es de resp
                  pendientes[xx,m,campo]=pendientes[xx,m,campo] - 1 
                  
                  if  t_entrada[xx, ag, m, campo, t] == t+tt:
                    hay =1                         
