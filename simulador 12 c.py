...
def CONSULTAR(X,Y,m,id_campo,nivel,Qx1,Qy):
  import random
  import numpy
      ...
      if restantes[Y] >0 :  # and MODO == "I" or MODO=="S":
        ...
        if ncons> 0 and not X == Y:
          ...
          #
            ...
            tipo_mens[X,Y,m,id_campo,t+espera]=1
            t_generac[X,Y,m,id_campo,t+espera]=t
            t_entrada[X,Y,m,id_campo, t]= t+espera
            f=id_campo
...
def BUSCOPADRES(Y, m, id_campo, lista): # Busca en  la cola de entrada de Y un mens de C con ese id msg y id campo y en lista da la lista de padres (de ag que consultaron)
  ...
  for ag in range (1,N_agentes):
    if ((ag,Y,m,id_campo) in   tipo_mens) and tipo_mens[ag,Y,m,id_campo]=="1":
      lista.append(ag)
...
def devuelvo_resp(X,Y,m, id_campo,nivel,Qy2):
  ...
  #  print("estoy en devuelvo resp  X="+str(X)+" Y= " +str(Y)+" id_campo "+str(id_campo))
    ...
    if restantes[X]>0 : #  and MODO == "I" or MODO=="S":
        ...
        tipo_mens[X,Y, m, id_campo,t+espera]=2
...
def ES_TO(Y,id_campo):
  ...
  for i in range (1,N_agentes):
    for j in range(0,t):
      if  tipo_mens[i,Y,m,id_campo,j] == 2 and t_generac[i,Y,m,id_campo,j] + TIMEOUT > t_entrada[i,Y,m,id_campo, t_generac[i,Y,m,decision,j]] :
        return False
      else:
...
def RESPUESTA_OMEGA(X,Y,m,id_campo,Qx,Qy,N_agentes, MODO):
 ...
 # CALCULO_Qy(X,Y,id_msg,id_campo,Qy)
  ...
  for i,p,m,id_campo,j in t_entrada:
    if p==1:
      if  t_entrada [i,1,m,id_campo,j] > maxt and tipo_mens[i,1,m,id_campo,t_entrada [i,1,m,id_campo,j]] == 2:
   #     print(t_entrada,"t es", str(t))
        maxt=t_entrada [i,1,m,id_campo,j]
...
def RESPONDO_PADRES(Y,m,id_campo, lista, Qy1,nivel):
 # print ("****Respondo a los padres de Y "+str(Y)+ " campo "+ str(id_campo)+" calidad "+ str(Qy1))
  ...
  for i in range(0,len(lista)-1):
      Qy1=devuelvo_resp(Y,lista[i],m,id_campo,nivel,Qy1)
...
# ***********************************************************************************
def RESPONDER(X,Y,m,id_campo,Qy,N_agentes,MODO, hora):
  global id_msg
  ...
  if (X,Y,m,id_campo,hora) in tipo_mens:
      if tipo_mens[X,Y,m,id_campo,hora]==2 and t_entrada [X,Y,m,id_campo,t_generac[Y,X,m,id_campo,hora] ] == hora or tipo_mens[X,Y,m,id_campo,hora]==1 and t_entrada [X,Y,m,id_campo,t_generac[Y,X,m,id_campo,hora] ] > t_generac[Y,X,m,id_campo,hora] + TIMEOUT:
         ...
         #         print(" Qy compleja es ", Qy)                                
            ...
            #   GENERO_RESP(Y,id_msg,id_campo, lista, Qy)
                for z in lista:
                  if restantes[Y]>0:
                    espera=int(random.random()*100)
                    t_entrada[Y,z,m,id_campo,t]=t+espera
                    MENSAJES_ITER=MENSAJES_ITER+1
                    t_generac[Y,z,m,id_campo,t+espera]=t
                    tipo_mens[Y,z,m,id_campo,t+espera]=2
  ...
  #      if pendientes[X,id_msg,id_campo]== 0:
   ...
   #       print("RESPONDER  valor de entrADA ",R, " resp ", str(respuestas[X,Y,id_msg,id_campo]))
        ...
        if t_entrada [X,Y,m,id_campo,t_generac[Y,X,m,id_campo,hora] ] > t_generac[Y,X,m,id_campo,hora] + TIMEOUT:
          TO_MSG[X,Y]=TO_MSG[X,Y]+1    
  return
...
def arma_lista_fav(x,c,l,N_favoritos,N_agentes):
  ...
  if MODO=="S":
    ...
    l= random.sample(range(1,N_agentes+1), kk)
    if x in l:
      l.remove(x)
    if 0 in l:
      l.remove(x)
...
def PROCESO_ARCH():
  SALTO=999
  import numpy, random
  global id_msg, MAXMENS, MAXREGS,N_agentes,MAXIT,MIN_CONF_O,MAX_CONF_O
  global t, decision,line111, RESTANTES_ANT,RESTANTES_ANT_REG
...
#  cuenta_to=0
  ...
  #llave = 1         COMENTE EL BLOQUE PARA EL DEBUGGING CON 1 SOLO REG. LLave dice si es el primer reg o no
    ...
    #  print("voy a armar lista fav")
      lista=[]
      lista1=arma_lista_fav(1,decision,lista,N_favoritos,N_agentes) 
  #    print(" favs de OMEGA para decision",lista1[1:10])
    ...
    #       Resp.write(rr)
       ...
       #   print ("OMEGA consulta a "+str(lista[i-2])+" con calidad "+str(Qx1))
          if m < MAXREGS+1:
            niveles[1,lista[i-2],m, decision]=nivel
        #     T=t
          if restantes[1] > 0  : # and MODO == "I" or MODO=="S":
            if i> 2 and m < MAXREGS + 1 :
              Qy=CONSULTAR(1, lista[i-2], m, decision, niveles[1,lista[i-2],m, decision], Qx1, Qy)  
      ...
      for xx, ag, m, campo,a in list(tipo_mens):
       ...
       #     print("no es T O y es repuesta de",xx," a", ag)
            ...
            if restantes[ag]>0 :           # and MODO == "I" or MODO=="S":
              RESPONDER(ag,xx,m,campo,Qaux,N_agentes,MODO,a)
  ...
  #    print("proceso mensajes de OMEGA en main()") 
    ...
    #  Resp.write(rr)
      ...
      # for p,pp,rr,decision,tie in list(t_entrada) and tipo_mens and pp==1 and tie==t:
          if (p,1,r,decision,t) in list(t_entrada) and  t_entrada[p,1,r,decision,t] == t+taux and (p,1,r,decision,t+taux) in list(tipo_mens) and tipo_mens[p,1,r,decision,t+taux]==2:
          ...
          #       print("---- Calidad de OMEGA ="+str(Qx1))
            if t_generac[p, 1, r, decision,t+taux] + TIMEOUT > t_entrada [p,1,r,decision,t]:
              ...
              #          print ("calidad Y"+str(Qy)+"calidad X "+str(Qx[1, p, r,decision]))       
                ...
                if abs(Q[p,1, m,decision] - Qx1)<0.0001:
                  ES_EXITO_A=1
              #       EXITOS_A =EXITOS_A+1
                
                  
            else:
                TO_MSG[p,1]=TO_MSG[p,1]+1
...
#                   for tiempo in range (0,t+TIMEOUT):
    uu=0
    NO=0
    for u,uu,m1,decision1,tiempo1 in list(tipo_mens): 
      ...
      if uu==1 and tiempo1 > t :   
     #   print ("encontre", u,1,m,decision,tiempo )
        if tipo_mens[u,1,m1,decision1,tiempo1]== 2 and  t_generac[u,1,m1,decision1,tiempo1] + TIMEOUT > t_entrada[u,1,m1,decision1, t_generac[u,1,m1,decision1,tiempo1]]  and m1==m and decision1==decision and tiempo1 > tiempo:
       #   print("************  NO ES T O")
          enhora=1
          NO=1
       #   print("Q es ", Qx[u,1,m,decision]," calidad de OMEGA=",Qx1)
          if Qx1 < Qx[u,1,m,decision]  :
            ESTRICTO=1
          else:
            if abs(Qx1- Qx[u,1,m,decision]) < 0.01:
              AMPLIO=1

        else:
          if t_generac[u,1,m1,decision1,tiempo1] + TIMEOUT < t_entrada[u,1,m1,decision1, t_generac[u,1,m1,decision1,tiempo1]] and m1==m and decision1==decision and tiempo1>tiempo:
...
#  import pandas
  ...
  global Resp
  global id_msg
  global MAXMENS,N_agentes, MAXIT,MAXREGS,MIN_CONF_O,MAX_CONF_O,N_campos
  global t_entrada, t_generac, tipo_mens, largo_cola, niveles, pendientes, restantes, MENSAJES_ITER
  ...
  global t
  global restantes
  TOLERANCIA=2 # cuantas veces se admite un to desde un favorito antes de borrarlo de la lista de favoritos
  # cant reg a prcesar por OMEGA en el archivo
  global MAXREGS
  MAXREGS= 32 
  
  #for N_agentes in (10,40):
  for N_agentes in (200,201):
    ...
    #  for MAXMENS in [100]: 
       ...
       # for MODO in ["S"]:
              ...
              for iteracion in range(1,MAXIT+1): # cada iteracion son los 50 regs
                  ...
                  id_msg=0
                  TIMEOUTS=0
                  global cuenta_to, cuenta_e, cuenta_o
                  cuenta_to=0
                  cuenta_a=0
                  cuenta_e=0              
          ...
          #       TABLA_TO[iteracion,N_agentes,MAXREGS,MAXMENS]=cuenta_to
              ...
              s="RESULTADOS_"+str(N_agentes)+"_"+str(MAXMENS)+"_"+MODO+"_"+str(MAXIT)+"_"+str(MAXREGS)+"_"+str(N_favoritos)+".txt"
