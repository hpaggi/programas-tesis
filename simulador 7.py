...
def devuelvo_resp(X,Y,id_msg,id_campo,nivel,Qy):
  ...
  #  print("estoy en devuelvo resp  X="+str(X)+" Y= " +str(Y)+" id_campo "+str(id_campo))
    ...
    if pendientes[Y,id_msg,id_campo] < 1 or ES_TO(Y,id_msg,id_campo):
      ...
      print("devuelvo resp",Qy,Qy1)
      
      suma=0
      for i in range (0,N_agentes):
        suma=suma+restantes[i]
        
      TABLA_CALIDADES[iteracion,int(Qy1*100)]=  RESTANTES_REG_ANT -suma
      RESTANTES_REG_ANT= suma - RESTANTES_REG_ANT 
...
def RESPONDER(X,Y,id_msg,id_campo,Qx,Qy,N_agentes,tipo_mens,MODO, restantes,hora):
  ...
  if (X,Y,id_msg,id_campo,t) in tipo_mens:
      if tipo_mens[X,Y,id_msg,id_campo,hora]==2 and t_entrada [X,Y,id_msg,id_campo,t] == hora or tipo_mens[X,Y,id_msg,id_campo,hora]==1 and t_entrada [X,Y,id_msg,id_campo,hora] > t_generac[Y,X,id_msg,id_campo,hora] + TIMEOUT:
...
#  import pandas
  ...
  global TABLA_MENS
  TABLA_EXITOS_E={}
  # cada fila tiene (iteracion,N_agentes,MAXREGS,MAXMENS), cada fila es una iterac
  TABLA_EXITOS_A={}
  # cada fila tiene (iteracion,N_agentes,MAXREGS,MAXMENS), cuenta EA cada fila es una iterac
  TABLA_MENS={}
    # cada fila tiene (iteracion,N_agentes,MAXREGS,MAXMENS), mensajes usados,  cada fila es una iterac
  global TABLA_CALIDADES
  TABLA_CALIDADES=numpy.zeros((40,100),dtype=int)
  # cada fila tiene cuantos mensajes se usaron para tener esa calidad final Q en la salida de OMEGA, cada fila es una iterac. 
  
  global TABLA_TO
  
  TABLA_TO=numpy.zeros((40,N_agentes,MAXREGS),dtype=int)
  ...
  while iteracion < 35:
        ...
        RESTANTES_REG_ANT= N_agentes*MAXMENS
          ...
          for i in range (1,tope):
            ...
            #        print ("OMEGA consulta a "+str(lista[i-2])+" con calidad "+str(Qx1))
               ...
               #     T=t
                    ...
                    print("calidad devuelta a OMEGA "+str(Qy) ," calidad OMEGA", Qx1)
                      ...
                      if (xx, ag, m, campo,t+tt) in tipo_mens:
                        ...
                        if t_generac[xx, ag, m, campo,t+tt] + TIMEOUT > t+tt and tipo_mens[xx, ag, m, campo,t+tt] == 2 and t_entrada[xx, ag, m, campo, t] == t+tt:  # o solo > t ?? NO ES TO 
                          ...
                          if t_generac[xx, ag, m, campo,t+tt] + TIMEOUT > t+tt and t_entrada[xx, ag, m, campo, t] == t+tt and tipo_mens[xx, ag, m, campo,t+tt] == 1 :    # 1 - consulta 2 - Respuesta :
                              print 
                              hay =1                         
                              ("no es T O y es consulta de",xx," a", ag)   
      ...
      #    print("EXITOS A ", str(EXITOS_A),"EXITOS E ", str(EXITOS_E), "TIME OUTS ", str(TIMEOUTS))
       ...
       # Resp.write(rr)
        ...
        TABLA_TO[iteracion,N_agents,MAXREGS]=cueta_to
        
        restan=0
        for i in range (1,N_agentes):
          restan=restan+restantes[i]
        TABLA_MENS.append(iteracion,N_agentes,MAXREGS,MAXMENS)
        TABLA_MENS[iteracion,N_agentes,MAXREGS,MAXMENS]=  RESTANTES_ANT-restan   
        
        iteracion=iteracion+1
        RESTANTES_ANT=RESTANTES_ANT-restan 
        
  print(TABLA_EXITOS_A)
  print(TABLA_EXITOS_E)
  print(TABLA_MENS)
  print(TABLA_TO)
