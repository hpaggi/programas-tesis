...
# el agente X consulta al Y por el campo id_campo
  ...
  rr="Estoy en CONSULTAR  campo "+" X "+str(X)+" Y "+str(Y)+" MSg " + str(id_msg) +" campo "+str(id_campo) + " nivel " +str(nivel)+ " Qx1"+str(Qx1)+" Qy "+str(Qy)
  rr=rr+"\n"
  print(rr)
  Resp.write(rr)  
  lista_subcampos=[]
  ccons=numpy.zeros(N_campos, dtype=int)
  S[X,id_msg,id_campo]=S[X,id_msg,id_campo]+1
  largo_cola[X,id_msg,id_campo]=largo_cola[X,id_msg,id_campo]+1
  ...
  k=0
  Z=0
  # si no tiene subcampos tot esta en 0 y ncons=0. Si no tiene subcampos asumo que no consulta.
  if ncons> 0 and not X == Y:
    ...
    if kk>0: 
          ...
          if restantes[Y] > 0:
            ...
            while  p < N_favoritos:  # en realidad habria que distinguir su ya se determinaron los favoritos y sino usar muchos como fin de loop
              ...
              if not Z == X:
                ...
                if not (Y,id_msg, ccons[k]) in S:
                  S[Y,id_msg, ccons[k]] = 0
                  largo_cola[Y, id_msg, ccons[k]] = 0
           ...
           # print("Genero Consulta de " +str(Y)+"a X "+str(X)+ " msg "+str(id_msg)+ " campo  "+ str(ccons[k]) + " cola  "+str(largo_cola[Y, id_msg, ccons[k]]) )
                ...
                t_entrada[X,Y,id_msg,id_campo, t+espera]= t+espera
               
                largo_cola[Z, id_msg, ccons[k]]= largo_cola[Z, id_msg, ccons[k]] +1
                niveles[Y, Z,id_msg, ccons[k]]=nivel+1
      ...
      Qy=1/(1+H[Y,id_campo])*(1/n**PESO_NIVEL)
...
def CALCULO_Qy(X,Y,id_msg,id_campo,Qy):
  ...
  for i in range (1, N_agentes):
    ...
    if Q[X,i,id_msg,id_campo] > Max:
      
      Max= Q[X,i,id_msg,id_campo]
  print("calculo Q. cal max recibida=",Max)
  for i in range (1, N_agentes):
    if Q[X,i,id_msg,id_campo] < Min:
      Min = Q[X,i,id_msg,id_campo]      
  ...
  a=Max/(niveles[X,Y,id_msg,id_campo]**PESO_NIVEL)
  V=Max-Min
  q=1/(1+V)*Max
  R=T[X,id_msg,id_campo]**2/S[X,id_msg,id_campo]
  if R==0:
    R=1/S[X,id_msg,id_campo]
  Qy=max(Qx[X,Y,id_msg,id_campo],q)**((PESO_NIVEL+1)/R)
  Q[X,Y,id_msg,id_campo]=Qy
  print("Calculo Qy="+str(Qy)+ " id msg "+ str(id_msg)+ " id campo " + str(id_campo))
  return
#************************************************************************************
...
def devuelvo_resp(X,Y,id_msg,id_campo,nivel,Qy):
    ...
    if pendientes[Y,id_msg,id_campo] < 1:
      ...
      #   genera una respuesta a partir de todas las calidades recibidas  +++++ fALTA+++  Esa respuesta es Qy1
      Qy1=0.96           
      # genera un mensaje para cada padre de Respuesta
      RESPONDO_PADRES(Y,id_msg,id_campo, lista, Qy1,nivel)
      RESPONDE_OMEGA=0 
      if Y == 1:
        print("**** respuesta de OMEGA al msg ", str(id_msg), " valor =", str(respuestas[X,Y,id_msg,id_campo]), "calidad",str(Qy1))
        RESPUESTA_OMEGA (1,1,id_msg,decision,Qx,Qy,N_agentes,tipo_mens,msg, MODO, restantes,hora)
    ...
    return
#************************************************************************************
def RESPUESTA_OMEGA(X,Y,id_msg,id_campo,Qx,Qy,N_agentes,tipo_mens,msg, MODO, restantes,hora):
  ...
  #for i in range (1,largo_cola[X, id_msg, id_campo]):
        ...
        #  a = restantes [X] 
            ...
            if Qx1 > Qy: 
              ...
              #tipo_mens[X,Y,id_msg,id_campo,t+espera]=2 #+++++ revisar
             ...
             #   BUSCOPADRES(Y,id_msg,id_campo, lista)
               ...
               # restantes[Y]=restantes[Y]-1    omega no tiene limites de mensajes
              
              if t < hora_mens[id_msg]+TIMEOUT:
...
# ***********************************************************************************
def RESPONDER(X,Y,id_msg,id_campo,Qx,Qy,N_agentes,tipo_mens,msg, MODO, restantes,hora):
  ...
  if (X,Y,id_msg,id_campo,t) in tipo_mens:
      if tipo_mens[X,Y,id_msg,id_campo,hora]==2 and t_entrada [X,Y,id_msg,id_campo,hora] == t or tipo_mens[X,Y,id_msg,id_campo,hora]==1 and t_entrada [X,Y,id_msg,id_campo,hora] == t + T_TIMEOUT:
...
#  import pandas
  ...
  MAXREGS= 4          # 40 es el numero real 
  ...
  FAVORITOS=numpy.zeros((N_agentes,N_agentes,N_campos), dtype=int)
...
#  tipo_mens=numpy.array((N_agentes,N_agentes, MAXREGS, N_campos, maxt), dtype=int)
 ...
 # respuestas=numpy.zeros((N_agentes,N_agentes, MAXREGS333933,N_campos, maxt), dtype=int) tiene sentido que respuestas dependan de t?? +++
  ...
  # S cantidad de agentes consultados
  S=numpy.zeros((N_agentes, MAXREGS,N_campos), dtype=int)
  
  # T cantidad de agentes que responden
  T=numpy.zeros((N_agentes, MAXREGS,N_campos), dtype=int)
  
  # los errores 
  for i in range(1,N_agentes):
    for j in range(0,N_campos):
      H[i,j]=1- min(random.random()+0.7, 0.999)
...
# Qx1 es la calidad de OMEGA     
        ...
        if restantes[1] > 0:
         #   print("Restantes de OMEGA"+str(restantes))
          ...
          #  print("voy a armar lista fav")
            ...
            rr="OMEGA consulta a "+str(lista[i-1])+" con calidad "+str(Qx1)+"\n"
            Resp.write(rr)
            print ("OMEGA consulta a "+str(lista[i-1])+" con calidad "+str(Qx1))
            niveles[1,lista[i-1],id_msg, decision]=nivel
       #     T=t
            
            CONSULTAR(1, lista[i-1], decision, id_msg, niveles[1,lista[i-1],id_msg, decision], Qy, Qx1, pendientes, N_campos, restantes)
   ...
   #mientras haya mensajes para algun agente
           ...
           # for campo in range (1,N_campos+1):
            for campo in range (1,73): 
              if (xx, ag, m, campo,t+tt) in tipo_mens:
                ...
                if t_generac[xx, ag, m, campo,t+tt] + T_TIMEOUT > t+tt:  # o solo > t ??+++
                  pendientes[xx,m,campo]=pendientes[xx,m,campo] - 1 
                  if  t_entrada[xx, ag, m, campo, t+tt] == t+tt:
                    hay =1                         
                    ...
                    if  tipo_mens[xx, ag, m, campo,t+tt] == 1 :    # 1 - consulta 2 - Respuesta
                      ...
                      CONSULTAR(xx, ag, campo, m, niveles[xx, ag, m, campo], Qy, Qaux, pendientes, N_campos, restantes)
                    else:
                      if  tipo_mens[xx, ag, m, campo, t+tt]==2 and  t_entrada[xx, ag, m, campo, t+tt] == t+tt :
                        # prceso respuesta
                        hay =1 
                        Qaux=Q[xx,ag,m,campo]
                        RESPONDER(ag,xx,m,campo,Qy,Qaux,N_agentes,tipo_mens,m, MODO, restantes,t+tt)
                else:
                    TIMEOUTS=TIMEOUTS+1
              campo=campo+1
      if hay ==1:
       ...
       # break
        ...
        while p < N_agentes+1:
          ...
          for r in range (1,MAXREGS+1):
            if (p,1,r,decision,t+taux) in t_entrada and  t_entrada[p,1,r,decision,t+taux] == t+taux and tipo_mens[p,1,r,decision,t+taux]==2:
              ...
              Qy=0.95
              if t_generac[p, 1, r, decision,t+taux] + T_TIMEOUT <= t_entrada [p,1,r,decision,t+taux]:
                # no es time out
                NO_ES_TO=1
                if Q[1, p, r,decision] > Qy:  
                  EXITOS_E =EXITOS_E+1
    ...
    if id_msg < MAXREGS +1 :
      t_entrada=largo_cola=pendientes=niveles={}
      Q=0
