...
def RESPONDER(X,Y,id_msg,id_campo,Qy,N_agentes,tipo_mens,MODO, restantes,hora):
  ...
  if (X,Y,id_msg,id_campo,hora) in tipo_mens:
      if tipo_mens[X,Y,id_msg,id_campo,hora]==2 and t_entrada [X,Y,id_msg,id_campo,t] == hora or tipo_mens[X,Y,id_msg,id_campo,hora]==1 and t_entrada [X,Y,id_msg,id_campo,t] > t_generac[Y,X,id_msg,id_campo,hora] + TIMEOUT:
  ...
  #      if pendientes[X,id_msg,id_campo]== 0:
   ...
   #       print("RESPONDER  valor de entrADA ",R, " resp ", str(respuestas[X,Y,id_msg,id_campo]))
        ...
        if t_entrada [X,Y,id_msg,id_campo,t] > t_generac[Y,X,id_msg,id_campo,hora] + TIMEOUT:
          TO_MSG[X,Y]=TO_MSG[X,Y]+1    
  return
...
#  import pandas
  ...
  TABLA_CALIDADES_MENS=numpy.zeros((40,100),dtype=int)
  TABLA_CALIDADES_FREC=numpy.zeros((40,100),dtype=int)
  ...
  MAXIT=3 
  for N_agentes in [10,20,30,40,50,100,200,300,1000]:
            ...
            while iteracion < MAXIT+1:
                ...
                #  tipo_mens=numpy.array((N_agentes,N_agentes, MAXREGS, N_campos, maxt), dtype=int)
                 ...
                 # largo_cola=numpy.zeros((N_agentes,MAXREGS, N_campos,maxt), dtype=int)
                  largo_cola=numpy.zeros((N_agentes,MAXREGS+1, N_campos), dtype=int)
                 ...
                 # respuestas=numpy.zeros((N_agentes,N_agentes, MAXREGS,N_campos, maxt), dtype=int) tiene sentido que respuestas dependan de t?? +++
                  global respuestas
                  respuestas=numpy.zeros((N_agentes,N_agentes, MAXREGS+1,N_campos), dtype=int)
                  # formado por
                  ...
                  global niveles  
                  niveles=numpy.zeros((N_agentes, N_agentes, MAXREGS+1, N_campos), dtype=int)
                  ...
                  Q=numpy.zeros((N_agentes,N_agentes,MAXREGS+1, N_campos), dtype=float)
                  
                  # S cantidad de agentes consultados cuando X consulta a Y
                  S=numpy.zeros((N_agentes, N_agentes, MAXREGS+1,N_campos), dtype=int)
                  
                  # T cantidad de agentes que responden
                  T=numpy.zeros((N_agentes, N_agentes, MAXREGS+1,N_campos), dtype=int)
                           ...
                           #     T=t
                                if restantes[1] > 0  and MODO == "I" or MODO=="S":
                                    if i> 2 and id_msg < MAXREGS + 1 :
                                     ...
                                     # print("calidad devuelta a OMEGA "+str(Qy) ," calidad propiade OMEGA", Qx1)
                            ...
                            for campo in range (1,N_campos+1):
                              #for campo in range (1,73): 
                                if (xx, ag, m, campo,t+tt) in tipo_mens:
                                  ...
                                  if t_generac[xx, ag, m, campo,t+tt] + TIMEOUT > t+tt and tipo_mens[xx, ag, m, campo,t+tt] == 2 and t_entrada[xx, ag, m, campo, t] == t+tt:  # o solo > t ?? NO ES TO 
                               ...
                               #     print("no es T O y es repuesta de",xx," a", ag)
                                    ...
                                    if restantes[ag]>0  and MODO == "I" or MODO=="S":
                                      RESPONDER(ag,xx,m,campo,Qaux,N_agentes,tipo_mens,MODO, restantes,t+tt)
                                      FAVORITOS[ag,xx,campo]=FAVORITOS[ag,xx,campo]+1
                                  else:  
                                    if t_generac[xx, ag, m, campo,t+tt] + TIMEOUT > t+tt and t_entrada[xx, ag, m, campo, t] == t+tt and tipo_mens[xx, ag, m, campo,t+tt] == 1 :    # 1 - consulta 2 - Respuesta :
                     ...
                     #  Resp.write(rr)
                       ...
                       #       print("---- Calidad de OMEGA ="+str(Qx1))
                              if t_generac[p, 1, r, decision,t+taux] + TIMEOUT <= t_entrada [p,1,r,decision,t]:
                                # no es time out
                                NO_ES_TO=1
                                if Q[p,1, r,decision] > Qx1  and MODO == "I" or MODO=="S":  
                                  ES_EXITO_E=1
                                #  EXITOS_E =EXITOS_E+1
                       ...
                       #   print (tipo_mens[u,1,id_msg,decision,tiempo],t_generac[u,1,id_msg,decision,tiempo],t_entrada[u,1,id_msg,decision, t_generac[u,1,id_msg,decision,tiempo]])
                         ...
                         #   print("************  NO ES T O")
                          ...
                          #  print("Q es ", Qx[u,1,id_msg,decision]," calidad de OMEGA=",Qx1)
                            if Qx1 < Qx[u,1,id_msg,decision]  and MODO == "I" or MODO=="S":
                              ESTRICTO=1
                            else:
             ...
             #      print("*************************** leo reg **************************")
                  ...
                  #    continue
                    ...
                    if id_msg < MAXREGS +1 :
                      ...
                      pendientes=numpy.zeros((N_agentes,MAXREGS+1, N_campos), dtype=int)
                      Q=numpy.zeros((N_agentes,N_agentes,MAXREGS+1, N_campos), dtype=float)
                      niveles=numpy.zeros((N_agentes, N_agentes+1, MAXREGS+1, N_campos), dtype=int)
                      TO_MSG=numpy.zeros((N_agentes, N_agentes), dtype=int)      
